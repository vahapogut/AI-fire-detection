...
const HistoryModal = ({ isOpen, onClose }: HistoryModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl w-full max-w-5xl h-[80vh] flex flex-col overflow-hidden shadow-2xl shadow-red-900/10">
        ...
        <div className="flex-1 overflow-auto p-6 flex gap-6">
           ...
           <div className="flex-1 overflow-y-auto custom-scrollbar">
             {loading ? (
                <div className="flex justify-center items-center h-full text-gray-500">{t.common.loading}</div>
             ) : events.length === 0 ? (
                <div className="flex justify-center items-center h-full text-gray-500">{t.history.noEvents}</div>
             ) : (
               <table className="w-full text-left text-sm text-gray-400">
                 <thead className="text-xs uppercase bg-white/5 text-gray-300 font-semibold sticky top-0 backdrop-blur-md">
                   <tr>
                     <th className="px-4 py-3 rounded-tl-lg">{t.history.time}</th>
                     <th className="px-4 py-3">{t.history.type}</th>
                     <th className="px-4 py-3">{t.history.confidence}</th>
                     <th className="px-4 py-3 text-right rounded-tr-lg">{t.history.action}</th>
                   </tr>
                 </thead>
              ...
              {selectedSnapshot ? (
                <div className="space-y-4 w-full">
                  ...
                  <div className="text-left">
                     <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">{t.history.selectedRecord}</p>
                ...
                <div className="text-gray-600 space-y-2">
                  ...
                  <p className="text-sm">{t.history.clickToView}</p>
                </div>
              )}
           </div>
        </div>
